...
import com.mitsolutions.yourbyke.config.KafkaProperties;
...
class YourBykeKafkaResourceIT {
    ...
    void setup() {
        ...
        YourBykeKafkaResource kafkaResource = new YourBykeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/your-byke-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/your-byke-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
